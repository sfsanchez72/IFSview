...
global var_contour,cont_min,cont_max,cont_level
...
def change_cmap(j):
    ...
    mod_cmap=plt.get_cmap(m)
#    if (invert==0):
#        mod_cmap=cmap_map(lambda x: pow((x*bright),2-2/(1+contrast)), plt.get_cmap(m))
#    else:
#        mod_cmap=cmap_map(lambda x: 1-pow((x*bright),2-2/(1+contrast)), plt.get_cmap(m))
...
#    fitsdata=fitsdata*mask
    if (fixY==0):
        mean=np.median(fitsdata)
        max_now=np.max(fitsdata)
        y1=(-1)*mean
        y2=max_now
...
#    print spectra
    if (Type==1):
#        specFig.imshow(spectra,cmap=plt.get_cmap(m),alpha=1,vmin=0,vmax=1.2,interpolation='nearest',aspect='auto')  
        mean=spectra.mean()
        levels=arange(0.01,3*mean,0.01*mean)
#        specFig.contourf(wmap,ypmap,spectra,levels,cmap=plt.get_cmap(m),alpha=1,interpolation='nearest',aspect='auto')    
        extent=np.zeros(4)
        extent[0]=crval
        extent[1]=crval+cdelt*(nz-1)
        extent[2]=0
        extent[3]=nsum-1
#        if (invert==0):
#            mod_cmap=cmap_map(lambda x: pow((x*bright),2-2/(1+contrast)), plt.get_cmap(m))
...
#        mod_cmap=cmap_map(lambda x: pow((x*bright),2-2/(1+contrast)), plt.get_cmap(m))
        if (fixY==0):
            specFig.imshow(spectra,interpolation='nearest',aspect='auto',extent=extent,vmin=-0.1*mean,vmax=3*mean,cmap=mod_cmap)    
        else:
            specFig.imshow(spectra,interpolation='nearest',aspect='auto',extent=extent,cmap=mod_cmap,vmin=Y1,vmax=Y2)

#        print 'NSUM=',nsum
        specFig.set_ylim(0,nsum-1)
#    else:
#        specFig.grid()
    ...
    if (event.button==3):
        if (not ((event.xdata>0) and (event.xdata<nx) and (event.ydata>0) and (event.ydata<ny))):
            if ((event.xdata>crval) and (event.xdata<(crval+cdelt*nz))):
                ...
                vW2.set(W2)                
                Slice_Range(W1,W2)
                #plot_3d(W1,W2,0.05,j)
                #plot_3d_fill(W1,W2,j)
                plot_3d_nofill(W1,W2,j)
                #plot_3d_scatter(W1,W2,0.05,j)
...
def setBC():
    ...
    global j,l,nx_med,ny_med,invert
    global mod_cmap
...
colormapFrame.pack(side=Tk.TOP, expand=1)
#LabelFrame(colormapFrame,text="Colormaps")
